...
const linkifier = new linkifyIt().tlds(tlds).set({ fuzzyIP: true })
const linkifierCards = new linkifyIt().tlds(tlds)
...
const isLinkSeparated = (text: string, link: Match) => {
	if (
...
const extractLinksInBrackets = (text: string) => {
...
export const getCardLinksFromText = (text: string): Match[] | null => {
	const disallowedSchemas = ['mailto:', 'ftp:', '//']
	const links = linkifierCards.match(extractLinksInBrackets(text))
	if (!links) return null
	return links.filter((link) => !disallowedSchemas.includes(link.schema))
}
