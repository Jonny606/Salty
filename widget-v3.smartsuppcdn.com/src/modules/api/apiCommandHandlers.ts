...
import { sendMessageWithAuthCheck, setMessengerInputValue } from '@/modules/textarea'
...
import {
	...
	validatePropertiesParam,
	validateSocketMockDataParam,
	validateStringParam,
...
export const executeChatSendCommand: ApiCommandHandler = (name: ApiCommand, param: unknown) => {
	if (!checkApiFeatureEnabled()) return
	if (!validateStringParam(name, param)) return
	setMessengerInputValue(param)
	void sendMessageWithAuthCheck()
}
...
export const executeContactDataCommand = (name: ApiCommand, param: unknown) => {
	if (!validatePropertiesParam(name, param)) return
	visitor.updateVisitorContactData(param)
}
...
export const executeReloadWithMockData: ApiCommandHandler = (name: ApiCommand, param: unknown) => {
	if (!validateSocketMockDataParam(name, param)) return
	void visitorClientProvider.reloadClient(param)
}

export const executeReload: ApiCommandHandler = (name: ApiCommand, param: unknown) => {
	if (!validateBooleanParam(name, param)) return
	void visitorClientProvider.reloadClient(undefined, param)
}
