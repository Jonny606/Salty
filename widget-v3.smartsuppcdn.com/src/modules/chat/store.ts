import { AccountStatus, AgentStatus, ChatStatus, RatingTarget } from '@smartsupp/websocket-client-visitor'
...
export const accountStatus = writable<Exclude<AccountStatus, 'offlineWithAi'>>(AccountStatus.Offline)
...
export const suggestedRating = writable<RatingTarget | null>(null)
...
export const setAccountStatus = (status: AccountStatus) => {
	if (status === AccountStatus.OfflineWithAI) {
		accountStatus.set(AccountStatus.Online)
	} else accountStatus.set(status)
}
